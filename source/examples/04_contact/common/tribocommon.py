...
def add_tribo_with_meshes(damping = 2, offset=0.02):
    ...
    H_init = lgsm.Displacement(-1,1,.15, 1, 0, 0, 0)
    ...
    desc.simple.physic.addFixedJoint(world, "Obstacle.joint", "Obstacle", lgsm.Displacement(2., 1, .5, 1, 0,0,0))
    ...
    for node_name in bodies_name[1:] + ["Obstacle"]:
        ...
        composite = desc.collision.addCompositeMesh(world.scene.physical_scene.collision_scene, node_name+".comp", offset=offset)
        ...
        graph_node      = desc.core.findInTree(world.scene.graphical_scene.root_node, node_name)
        phy_node        = desc.physic.findInPhysicalScene(world.scene.physical_scene, node_name)
        graph_node.name = node_name # it is suitable to have the same name for both graphics and physics.

        phy_node.rigid_body.composite_name=node_name+".comp"
...
def addGround(world):
    ...
    desc.simple.physic.addFixedJoint(world, "ground.joint", "ground", lgsm.Displacementd(0,0,-0.1, 1,0,0,0)) #lgsm.Displacementd(1,0,-0.6, .95,0,.05,0))
